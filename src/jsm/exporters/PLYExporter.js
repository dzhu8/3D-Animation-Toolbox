import { Matrix3, Vector3, Color, ColorManagement, SRGBColorSpace } from "three";

/**
 * An exporter for PLY.
 *
 * PLY (Polygon or Stanford Triangle Format) is a file format for efficient delivery and loading of simple, static 3D
 * content in a dense format. Both binary and ascii formats are supported. PLY can store vertex positions, colors,
 * normals and uv coordinates. No textures or texture references are saved.
 *
 * ```js
...
class PLYExporter {
      ...
      * @returns {?(string | ArrayBuffer)} The exported PLY.
      */
 ...
 * @property {boolean} [binary=false] - Whether to export in binary format or ASCII. Default is `false`
 * @property {string[]} [excludeAttributes] - Which properties to explicitly exclude from the exported PLY file. Valid
 *   values are `'color'`, `'normal'`, `'uv'`, and `'index'`. If triangle indices are excluded, then a point cloud is
 *   exported.
 * @property {boolean} [littleEndian=false] - Whether the binary export uses little or big endian. Default is `false`
 */
