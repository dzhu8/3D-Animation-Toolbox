 ...
 * An exporter for EXR.
 *
 * EXR ( Extended Dynamic Range) is an [open format
 * specification]{@link https://github.com/AcademySoftwareFoundation/openexr} for professional-grade image storage format
 * of the motion picture industry. The purpose of format is to accurately and efficiently represent high-dynamic-range
 * scene-linear image data and associated metadata. The library is widely used in host application software where
 * accuracy is critical, such as photorealistic rendering, texture access, image compositing, deep compositing, and DI.
 *
 * ```js
...
class EXRExporter {
      ...
      * - When exporting a render target (e.g. a PMREM), it receives three parameters. The renderer, the render target and
      ...
      * @returns {Promise<Uint8Array>} A Promise that resolves with the exported EXR.
      */
     async parse(arg1, arg2, arg3) {
          if (!arg1 || !(arg1.isWebGLRenderer || arg1.isWebGPURenderer || arg1.isDataTexture)) {
               throw Error(
                    "EXRExporter.parse: Unsupported first parameter, expected instance of WebGLRenderer, WebGPURenderer or DataTexture."
               );
 ...
 * @property {HalfFloatType | FloatType} [type=HalfFloatType] - Output data type. Default is `HalfFloatType`
