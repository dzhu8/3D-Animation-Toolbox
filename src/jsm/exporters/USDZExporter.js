...
class USDZExporter {
      ...
      * @param {Object3D} scene - The 3D object to export.
      * @param {USDZExporter~Options} options - The export options.
      * @returns {Promise<ArrayBuffer>} A Promise that resolved with the exported USDZ data.
      */
...
function buildMaterial(material, textures, quickLookCompatible = false) {
     ...
     if (material.aoMap !== null) {
          ...
          samplers.push(
               buildTexture(
                    material.aoMap,
                    "occlusion",
                    new Color(material.aoMapIntensity, material.aoMapIntensity, material.aoMapIntensity)
               )
          );
     }
     ...
     if (material.roughnessMap !== null) {
          ...
          samplers.push(
               buildTexture(
                    ...
                    "roughness",
                    new Color(material.roughness, material.roughness, material.roughness)
               )
          );
     } else {
     ...
     if (material.metalnessMap !== null) {
          ...
          samplers.push(
               buildTexture(
                    ...
                    "metallic",
                    new Color(material.metalness, material.metalness, material.metalness)
               )
          );
     } else {
     ...
     if (material.isMeshPhysicalMaterial) {
          if (material.clearcoatMap !== null) {
               inputs.push(
                    `${pad}float inputs:clearcoat.connect = </Materials/Material_${material.id}/Texture_${material.clearcoatMap.id}_clearcoat.outputs:r>`
               );
               samplers.push(
                    buildTexture(
                         material.clearcoatMap,
                         "clearcoat",
                         new Color(material.clearcoat, material.clearcoat, material.clearcoat)
                    )
               );
          } else {
 ...
 * @property {number} [maxTextureSize=1024] - The maximum texture size that is going to be exported. Default is `1024`
 * @property {boolean} [includeAnchoringProperties=true] - Whether to include anchoring properties or not. Default is
 *   `true`
 * @property {Object} [ar] - If `includeAnchoringProperties` is set to `true`, the anchoring type and alignment can be
 *   configured via `ar.anchoring.type` and `ar.planeAnchoring.alignment`.
 * @property {boolean} [quickLookCompatible=false] - Whether to make the exported USDZ compatible to QuickLook which
 *   means the asset is modified to accommodate the bugs FB10036297 and FB11442287 (Apple Feedback). Default is `false`
 */
