 ...
 * navigation functionality. Cursor/finger positions and movements are mapped over a virtual trackball surface
 * represented by a gizmo and mapped in intuitive and consistent camera movements. Dragging cursor/fingers will cause
 * camera to orbit around the center of the trackball in a conservative way (returning to the starting point will make
 * the camera return to its starting orientation).
 *
 * In addition to supporting pan, zoom and pinch gestures, Arcball controls provide focus< functionality with a double
 * click/tap for intuitively moving the object's point of interest in the center of the virtual trackball. Focus allows
 * a much better inspection and navigation in complex environment. Moreover Arcball controls allow FOV manipulation (in
 * a vertigo-style method) and z-rotation. Saving and restoring of Camera State is supported also through clipboard (use
 * ctrl+c and ctrl+v shortcuts for copy and paste the state).
 *
 * Unlike {@link OrbitControls} and {@link TrackballControls}, `ArcballControls` doesn't require `update()` to be called
 * externally in an animation loop when animations are on.
 *
 * @augments Controls
...
class ArcballControls extends Controls {
      ...
      *
      * @param {Camera} camera - The camera to be controlled. The camera must not be a child of another object, unless
      *   that object is the scene itself.
      * @param {HTMLDOMElement | null} [domElement=null] - The HTML element used for event listeners. Default is `null`
      * @param {Scene | null} [scene=null] The scene rendered by the camera. If not given, gizmos cannot be shown.
      *   Default is `null`
      */
     constructor(camera, domElement = null, scene = null) {
          ...
          /**
           * Holds the mouse actions of this controls. This property is maintained by the methods `setMouseAction()` and
           * `unsetMouseAction()`.
           *
           * @type {Object[]}
           */
          ...
          /**
           * If set to `true`, the camera's near and far values will be adjusted every time zoom is performed trying to
           * maintain the same visible portion given by initial near and far values. Only works with perspective
           * cameras.
           *
           * @default false
           * @type {boolean}
      ...
      *
      * @param {"PAN" | "ROTATE" | "ZOOM" | "FOV"} operation - The operation to be performed ('PAN', 'ROTATE', 'ZOOM',
      *   'FOV').
      * @param {0 | 1 | 2 | "WHEEL"} mouse - A mouse button (0, 1, 2) or 'WHEEL' for wheel notches.
      * @param {"CTRL" | "SHIFT" | null} [key=null] - The keyboard modifier ('CTRL', 'SHIFT') or null if key is not
      ...
      * @param {number} [amount=1] - Amount of operation to be completed (used for focus animations, default is complete
      *   full operation). Default is `1`
      */
      ...
      *
      * @private
      * @param {number} t - The absolute progress of the animation in the bound of `0` (beginning of the) and `1`
      *   (ending of animation).
     ...
     /**
      * Calculates the cursor position inside the canvas x/y coordinates with the origin being in the center of the
      * canvas.
      *
      * @private
      * @param {number} cursorX - Cursor horizontal coordinate within the canvas.
     ...
     pan(p0, p1, adjust = false) {
          ...
          if (this.object.isOrthographicCamera) {
               ...
               const distanceFactor =
                    this._v3_1.distanceTo(this._v3_2) / this.object.position.distanceTo(this._gizmos.position);
      ...
      * @returns {Object} Object with 'camera' field containing transformation matrix resulting from the operation to be
      ...
      * @returns {Object} Object with 'camera' and 'gizmo' fields containing transformation matrices resulting from the
      *   operation to be applied to the camera and gizmos.
      */
      ...
      *
      * @private
      * @param {Matrix4} [camera=null] - Transformation to be applied to the camera. Default is `null`
      * @param {Matrix4} [gizmos=null] - Transformation to be applied to gizmos. Default is `null`
      */
     ...
     /**
      * Returns the raycaster that is used for user interaction. This object is shared between all instances of
      * `ArcballControls`.
      *
      ...
      * @param {number} cursorY - Cursor vertical coordinate on screen.
      * @param {HTMLElement} canvas - The canvas where the renderer draws its output.
      * @param {boolean} [initialDistance=false] - If initial distance between camera and gizmos should be used for
      *   calculations instead of current (Perspective only). Default is `false`
