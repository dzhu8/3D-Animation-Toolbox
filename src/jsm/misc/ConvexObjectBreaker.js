 ...
 *
 * Use the function prepareBreakableObject to prepare a Mesh object to be broken. Then, call the various functions to
 * subdivide the object (subdivideByImpact, cutByPlane). Sub-objects that are product of subdivision don't need
 * prepareBreakableObject to be called on them.
 *
 * Requisites for the object:
 *
 * - Mesh object must have a buffer geometry and a material.
 * - Vertex normals must be planar (not smoothed).
 * - The geometry must be convex (this is not checked in the library). You can create convex geometries with
 *   {@link ConvexGeometry}. The {@link BoxGeometry}, {@link SphereGeometry} and other convex primitives can also be used.
 *
 * Note: This lib adds member variables to object's userData member (see prepareBreakableObject function) Use with
 * caution and read the code when using with other libs.
 *
...
class ConvexObjectBreaker {
      ...
      *
      * @param {number} [minSizeForBreak=1.4] - Min size a debris can have to break. Default is `1.4`
      * @param {number} [smallDelta=0.0001] - Max distance to consider that a point belongs to a plane. Default is
      *   `0.0001`
      */
     ...
     /**
      * Subdivides the given 3D object into pieces by an impact (meaning another object hits the given 3D object at a
      * certain surface point).
      *
      ...
      * @returns {Object3D[]} The array of pieces.
      */
