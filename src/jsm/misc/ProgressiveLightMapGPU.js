 ...
 *
 * To use, simply construct a `ProgressiveLightMap` object, `plmap.addObjectsToLightMap(object)` an array of semi-static
 * objects and lights to the class once, and then call `plmap.update(camera)` every frame to begin accumulating lighting
 * samples.
 *
 * This should begin accumulating lightmaps which apply to your objects, so you can start jittering lighting to achieve
 * the texture-space effect you're looking for.
 *
 * This class can only be used with {@link WebGPURenderer}. When using {@link WebGLRenderer}, import from
...
class ProgressiveLightMap {
      ...
      * @param {number} [resolution=1024] - The side-long dimension of the total lightmap. Default is `1024`
      */
      ...
      * @param {number} [blendWindow=100] - When >1, samples will accumulate over time. Default is `100`
      * @param {boolean} [blurEdges=true] - Whether to fix UV Edges via blurring. Default is `true`
      */
