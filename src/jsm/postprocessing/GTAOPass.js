...
class GTAOPass extends Pass {
      ...
      * @param {number} [width=512] - The width of the effect. Default is `512`
      * @param {number} [height=512] - The height of the effect. Default is `512`
     ...
     /**
      * Configures the GBuffer of this pass. If no arguments are passed, the pass creates an internal render target for
      * holding depth and normal data.
      *
      ...
      *
      * @param {Box3 | null} box - The AABB enclosing the scene that should receive AO. When passing `null`, to clip box
      *   is used.
      */
     setSceneClipBox(box) {
          if (box) {
     ...
     updateGtaoMaterial(parameters) {
          ...
          if (
               parameters.screenSpaceRadius !== undefined &&
               (parameters.screenSpaceRadius ? 1 : 0) !== this.gtaoMaterial.defines.SCREEN_SPACE_RADIUS
          ) {
     ...
     updatePdMaterial(parameters) {
          ...
          if (updateShader) {
               ...
               this.pdMaterial.defines.SAMPLE_VECTORS = generatePdSamplePointInitializer(
                    this.pdSamples,
                    this.pdRings,
                    this.pdRadiusExponent
               );
