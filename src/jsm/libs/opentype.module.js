/**
 * https://opentype.js.org v1.3.4 | (c) Frederik De Bleser and other contributors | MIT License | Uses tiny-inflate by
 * Devon Govett and string.prototype.codepointat polyfill by Mathias Bynens
 */
...
/**
 * A bounding box is an enclosing box that describes the smallest measure within which all the points lie. It is used to
 * calculate the bounding box of a glyph or text path.
 *
...
/**
 * Add the point to the bounding box. The x1/y1/x2/y2 coordinates of the bounding box will now encompass the given
 * point.
 *
...
/**
 * Add a X coordinate to the bounding box. This extends the bounding box to include the X coordinate. This function is
 * used internally inside of addBezier.
 *
 * @param {number} x - The X coordinate of the point.
 */
...
/**
 * Add a Y coordinate to the bounding box. This extends the bounding box to include the Y coordinate. This function is
 * used internally inside of addBezier.
 *
 * @param {number} y - The Y coordinate of the point.
 */
...
/**
 * Add a Bézier curve to the bounding box. This extends the bounding box to include the entire Bézier.
 *
...
/**
 * Add a quadratic curve to the bounding box. This extends the bounding box to include the entire quadratic curve.
 *
 * @param {number} x0 - The starting X coordinate.
 * @param {number} y0 - The starting Y coordinate.
...
/**
 * A bézier path containing a set of path commands similar to a SVG path. Paths can be drawn on a context using `draw`.
 *
 * @class
 * @class
 ...
 *
 * @param {number} [decimalPlaces=2] - The amount of decimal places for floating-point values. Default is `2`
 * @returns {string}
 */
...
/**
 * Convert a signed number between -32768 and +32767 to a three-byte value. This ensures we always use three bytes, but
 * is not the most compact format.
 *
 * @param {number}
 * @returns {Array}
 */
...
/**
 * Convert a signed number between -(2^31) and +(2^31-1) to a five-byte value. This is useful if you want to be sure you
 * always use four bytes, at the expense of wasting a few bytes for smaller numbers.
 *
 * @param {number}
 * @returns {Array}
 */
...
/**
 * Decodes an old-style Macintosh string. Returns either a Unicode JavaScript string, or 'undefined' if the encoding is
 * unsupported. For example, we do not support Chinese, Japanese or Korean because these would need large mapping
 * tables.
 *
...
/**
 * Encodes an old-style Macintosh string. Returns a byte array upon success. If the requested encoding is unsupported,
 * or if the input string contains a character that cannot be expressed in the encoding, the function returns
 * 'undefined'.
 *
 * @param {string} str
 * @param {string} encoding
 ...
 *
 * Variation adjustment deltas are used in ‘gvar’ and ‘cvar’ tables. They indicate how points (in ‘gvar’) or values (in
 * ‘cvar’) get adjusted when generating instances of variation fonts.
 *
 * @param {Array}
 * @returns {Array}
...
/**
 * Convert an object to a CFF DICT structure. The keys should be numeric. The values should be objects containing name /
 * type / value.
 *
 * @param {Object} m
 * @returns {Array}
...
/**
 * Convert a table object to bytes. A table contains a list of fields containing the metadata (name, type and default
 * value). The table itself has the field values set as attributes.
 *
...
/**
 * Parse a list of items. Record count is optional, if omitted it is read from the stream. itemCallback is one of the
 * Parser methods.
 */
...
/**
 * Parse a list of records. Record count is optional, if omitted it is read from the stream. Example of
 * recordDescription: { sequenceIndex: Parser.uShort, lookupListIndex: Parser.uShort }
 */
...
/**
 * Parse a GPOS valueRecord https://docs.microsoft.com/en-us/typography/opentype/spec/gpos#value-record valueFormat is
 * optional, if omitted it is read from the stream.
 */
...
/**
 * Parse a list of GPOS valueRecords https://docs.microsoft.com/en-us/typography/opentype/spec/gpos#value-record
...
/**
 * Parse a list of offsets to lists of 16-bit integers, or a list of offsets to lists of offsets to any kind of items.
 * If itemCallback is not provided, a list of list of UShort is assumed. If provided, itemCallback is called on each
 * item and must parse the item. See examples in tables/gsub.js
 */
...
/**
 * This is the encoding used for fonts created from scratch. It loops through all glyphs and finds the appropriate
 * unicode value. Since it's linear time, other encodings will be faster.
 *
 * @class
 * @class
 * @param {opentype.Font}
 ...
 *
 * @param {number} [x=0] - Horizontal position of the beginning of the text. Default is `0`
 * @param {number} [y=0] - Vertical position of the _baseline_ of the text. Default is `0`
 * @param {number} [fontSize=72] - Font size in pixels. We scale the glyph units by `1 / unitsPerEm * fontSize`. Default
 *   is `72`
...
/**
 * Split the glyph into contours. This function is here for backwards compatibility, and to provide raw access to the
...
/**
 * Draw lines indicating important font measurements. Black lines indicate the origin of the coordinate system (point
 * 0,0). Blue lines indicate the glyph bounding box. Green line indicates the advance width of the glyph.
 *
 * @param {CanvasRenderingContext2D} ctx - A 2D drawing context, like Canvas.
 * @param {number} [x=0] - Horizontal position of the beginning of the text. Default is `0`
 * @param {number} [y=0] - Vertical position of the _baseline_ of the text. Default is `0`
...
/**
 * A GlyphSet represents all glyphs available in the font, but modelled using a deferred glyph loader, for retrieving
 * glyphs only once they are absolutely necessary, to keep the memory footprint down.
 *
 * @class
 * @param {opentype.Font}
 * @param {Array}
...
/**
 * Generate a stub glyph that can be filled with all metadata _except_ the "points" and "path" properties, which must be
 * loaded only once the glyph's path is actually requested for text shaping.
 *
 * @param {opentype.Font} font
 * @param {number} index
...
Layout.prototype = {
     ...
     /**
      * Returns the best bet for a script name. Returns 'DFLT' if it exists. If not, returns 'latn' if it exists. If
      * neither exist, returns undefined.
      */
      ...
      *
      * @param {string} [script='DFLT'] Default is `'DFLT'`
      ...
      *
      * @param {string} [script='DFLT'] Default is `'DFLT'`
      * @param {string} [language='dlft'] Default is `'dlft'`
     ...
     /**
      * Returns the list of glyph indexes of a coverage table. Format 1: the list is stored raw Format 2: compact list as
      * range records.
      *
 ...
 *
 * @param {string} [script='DFLT'] - Use font.position.getDefaultScriptName() for a better default value. Default is
 *   `'DFLT'`
 * @param {string} [language='dflt'] Default is `'dflt'`
 * @returns {object[]} The list of kerning lookup tables (may be empty), or undefined if there is no GPOS table (and we
 *   should use the kern table)
 */
...
/**
 * List all ligatures (lookup type 4) for a given script, language, and feature. The result is an array of ligature
 * objects like { sub: [ids], by: id }
 *
...
/**
 * Add or modify a single substitution (lookup type 1) Format 2, more flexible, is always used.
 *
 * @param {string} feature - 4-letter feature name ('liga', 'rlig', 'dlig'...)
...
/**
 * Add a ligature (lookup type 4) Ligatures with more components must be stored ahead of those with fewer components in
 * order to be found
 *
 * @param {string} feature - 4-letter feature name ('liga', 'rlig', 'dlig'...)
 ...
 *
 * Fv ... refers to freedom vector pv ... refers to projection vector rp ... refers to reference point p ... refers to
 * to point being operated on d ... refers to distance
 *
 * # SETRELATIVE:
 *
 ...
 * -' ' b rp ' ' ' p _----------_-------------- (fv) pm
 *
 * Rpdx = rpx + d * pv.x rpdy = rpy + d * pv.y
 *
 * Equation of line b
 *
 * Y - rpdy = pvns * (x- rpdx)
 ...
 * Rpdx = rpx + d * pv.x rpdy = rpy + d * pv.y
 *
 * Equation of line b: pvns ... normal slope to pv
 *
 * Y - rpdy = pvns * (x - rpdx)
 *
 * X = p.x
 ...
 *     y - rpdy = pvns * (x - rpdx)
 *
 * Equation of freedom vector line: fvs ... slope of freedom vector (=fy/fx)
 *
 *     y - py = fvs * (x - px)
 *
 ...
 *
 * The weight of the movement of the reference point gets bigger the further the other reference point is away, thus the
 * safest option (that is avoiding 0/0 divisions) is to weight the original distance of the other point by the sum of
 * both distances.
 *
 * If the sum of both distances is 0, then move the point by the arithmetic average of the movement of both reference
 * points.
 *
 *            (+6)
 *     rp1o *---->*rp1
 ...
 *
 * @param {array} RUDs Replace, update and delete operations TODO: Perf. Optimization (lengthBefore === lengthAfter ?
 *   dispatch once)
 */
...
/**
 * Get an item at an offset from the current value example (current value is 3): 1 2 [3] 4 5 | items values -2 -1 0 1 2
 * | offset values
 *
 ...
 * @param {function} contextEndCheck A predicate function that returns true on context end TODO: call tokenize on
 *   registration to update context ranges with the new context.
 */
...
/** Infer bidirectional properties for a given text and apply the corresponding layout rules. */
...
/**
 * Store essential context checks: arabic word check for applying gsub features arabic sentence check for adjusting
 * arabic layout
 */
...
/** Perform pre tokenization procedure then tokenize text input */
...
/** Reverse arabic sentence layout TODO: check base dir before applying adjustments - priority low */
...
/**
 * Process a string of text to identify and adjust bidirectional text entities.
 *
...
/**
 * Convert the given text to a list of Glyph objects. Note that there is no strict one-to-one mapping between characters
 * and glyphs, so the list of returned glyphs can be larger or smaller than the length of the given string.
 *
 * @param {string}
...
/**
 * Retrieve the value of the kerning pair between the left glyph (or its index) and the right glyph (or its index). If
 * no kerning pair is found, return 0. The kerning value gets added to the advance width when calculating the spacing
 * between glyphs. For GPOS kerning, this method uses the default script and language, which covers most use cases. To
 * have greater control, use font.position.getKerningValue .
 *
 ...
 * @property {string} [language='dflt'] - Language system used to determine which features to apply. See
 *   https://www.microsoft.com/typography/developers/opentype/languagetags.aspx. Default is `'dflt'`
 * @property {boolean} [kerning=true] - Whether to include kerning values. Default is `true`
 * @property {object} [features] - OpenType Layout feature tags. Used to enable or disable the features of the given
 *   script/language system. See https://www.microsoft.com/typography/otspec/featuretags.htm
 */
Font.prototype.defaultRenderOptions = {
     kerning: true,
     features: [
          /**
           * These 4 features are required to render Arabic text properly and shouldn't be turned off when rendering
           * arabic text.
           */
...
/**
 * Helper function that invokes the given callback for each glyph in the given text. The callback gets `(glyph, x, y,
 * fontSize, options)`.* @param {string} text
 *
 ...
 *
 * This is something different than Path.getBoundingBox() as for example a suffixed whitespace increases the
 * advanceWidth but not the bounding box or an overhanging letter like a calligraphic 'f' might have a quite larger
...
/**
 * Draw the points of all glyphs in the text. On-curve points will be drawn in blue, off-curve points will be drawn in
 * red.
 *
 * @param {CanvasRenderingContext2D} ctx - A 2D drawing context, like Canvas.
 * @param {string} text - The text to create.
...
/**
 * Draw lines indicating important font measurements for all glyphs in the text. Black lines indicate the origin of the
 * coordinate system (point 0,0). Blue lines indicate the glyph bounding box. Green line indicates the advance width of
 * the glyph.
 *
 * @param {CanvasRenderingContext2D} ctx - A 2D drawing context, like Canvas.
 * @param {string} text - The text to create.
...
/**
 * Convert the font object to a SFNT data structure. This structure contains all the necessary tables and metadata to
 * create a binary OTF file.
 *
...
/**
 * Loads a font from a URL. The callback throws an error message as the first parameter if it fails and the font as an
 * ArrayBuffer in the second parameter if it succeeds.
 *
...
/**
 * Parse the OpenType file data (as an ArrayBuffer) and return a Font object. Throws an error if the font could not be
 * parsed.
 *
 * @param {ArrayBuffer}
...
/**
 * Asynchronously load the font from a URL or a filesystem. When done, call the callback with two arguments `(err,
 * font)`. The `err` will be null on success, the `font` is a Font object. We use the node.js callback convention so
 * that opentype.js can integrate with frameworks like async.js.
 *
