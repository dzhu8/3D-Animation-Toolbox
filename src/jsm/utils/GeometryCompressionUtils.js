...
function compressNormals(geometry, encodeMethod) {
     ...
     if (normal.itemSize != 3) {
          console.error(
               "THREE.GeometryCompressionUtils.compressNormals(): normal.itemSize is not 3, which cannot be encoded."
          );
     }
...
function compressPositions(geometry) {
     ...
     if (!position) {
          console.error(
               "THREE.GeometryCompressionUtils.compressPositions(): Geometry must contain position attribute."
          );
     }
     ...
     if (position.itemSize != 3) {
          console.error(
               "THREE.GeometryCompressionUtils.compressPositions(): position.itemSize is not 3, which cannot be packed."
          );
     }
