     ...
     init() {
          ...
          for (let i = 0; i < this.tiles.length; i++) {
               ...
               display.material.vertexNode = Fn(() => {
                    ...
                    const xBase = float(-1.0)
                         .add(cornerOffset)
                         .add(displaySize.div(2).div(aspectRatio))
                         .add(float(col).mul(displaySize.div(aspectRatio).add(margin)));

                    const yBase = float(1.0)
                         .sub(cornerOffset)
                         .sub(displaySize.div(2))
                    ...
                    const finalPos = vec2(scaledPos.x.add(xBase), scaledPos.y.add(yBase));
               ...
               display.material.outputNode = Fn(() => {
                    ...
                    return vec4(vec3(r, g, b).mul(sampledDepth).saturate().rgb, 1.0);
               })();
               ...
               if (this.tileShadowNode._shadowNodes[i] && this.tileShadowNode._shadowNodes[i].shadow) {
                    ...
                    console.warn(
                         `TileShadowNodeHelper: Could not create CameraHelper for tile index ${i}. Shadow node or camera missing.`
                    );
