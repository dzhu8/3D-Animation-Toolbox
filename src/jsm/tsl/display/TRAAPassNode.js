...
class TRAAPassNode extends PassNode {
     ...
     setSize(width, height) {
          ...
          if (
               this.renderTarget.width !== this._sampleRenderTarget.width ||
               this.renderTarget.height !== this._sampleRenderTarget.height
          ) {
     ...
     setup(builder) {
          ...
          const resolve = Fn(() => {
               ...
               const compressedCurrent = currentColor.mul(
                    float(1).div(max(currentColor.r, currentColor.g, currentColor.b).add(1.0))
               );
               const compressedHistory = clampedHistoryColor.mul(
                    float(1).div(max(clampedHistoryColor.r, clampedHistoryColor.g, clampedHistoryColor.b).add(1.0))
               );
               ...
               return add(currentColor.mul(currentWeight), clampedHistoryColor.mul(historyWeight)).div(
                    max(currentWeight.add(historyWeight), 0.00001)
               );
          });
