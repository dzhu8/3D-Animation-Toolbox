...
import { nodeObject, Fn, uniform, convertToTexture, float, vec4, uv, NodeUpdateType } from "three/tsl";

/**
 * Post processing node for applying chromatic aberration effect. This effect simulates the color fringing that occurs
 * in real camera lenses by separating and offsetting the red, green, and blue channels.
 *
 * @augments TempNode
 ...
 * @param {Node | number} [strength=1.0] - The strength of the chromatic aberration effect as a node or value. Default
 *   is `1.0`
 * @param {Node | Vector2} [center=null] - The center point of the effect as a node or value. If null, uses screen
 *   center (0.5, 0.5). Default is `null`
 * @param {Node | number} [scale=1.1] - The scale factor for stepped scaling from center as a node or value. Default is
 *   `1.1`
