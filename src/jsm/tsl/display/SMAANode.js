...
class SMAANode extends TempNode {
     ...
     constructor(textureNode) {
           ...
           * @type {RenderTarget}
           */
          this._renderTargetWeights = new RenderTarget(1, 1, {
               depthBuffer: false,
               type: HalfFloatType,
          });
           ...
           * @type {RenderTarget}
           */
          this._renderTargetBlend = new RenderTarget(1, 1, {
               depthBuffer: false,
               type: HalfFloatType,
          });
     ...
     setup(builder) {
          ...
          const SMAAEdgeDetectionVS = Fn(() => {
               const vOffset0 = vec4(uvNode.xy, uvNode.xy).add(
                    vec4(this._invSize.xy, this._invSize.xy).mul(vec4(-1.0, 0.0, 0.0, -1.0))
               );
               const vOffset1 = vec4(uvNode.xy, uvNode.xy).add(
                    vec4(this._invSize.xy, this._invSize.xy).mul(vec4(1.0, 0.0, 0.0, 1.0))
               );
               const vOffset2 = vec4(uvNode.xy, uvNode.xy).add(
                    vec4(this._invSize.xy, this._invSize.xy).mul(vec4(-2.0, 0.0, 0.0, -2.0))
               );
               ...
               Loop(
                    ...
                    () => {
                         ...
                         If(
                              coord.x
                                   .lessThanEqual(end)
                                   .or(e.g.lessThanEqual(float(0.8281)).or(e.r.notEqual(float(0)))),
                              () => {
                                   Break();
                              }
                         );
               ...
               Loop(
                    ...
                    () => {
                         ...
                         If(
                              coord.y
                                   .lessThanEqual(end)
                                   .or(e.r.lessThanEqual(float(0.8281)).or(e.g.notEqual(float(0)))),
                              () => {
                                   Break();
                              }
                         );
          ...
          const SMAAWeightsVS = Fn(() => {
               ...
               const vOffset0 = vec4(uvNode.xy, uvNode.xy)
                    .add(vec4(this._invSize.xy, this._invSize.xy).mul(vec4(-0.25, -0.125, 1.25, -0.125)))
                    .toVar();
               const vOffset1 = vec4(uvNode.xy, uvNode.xy)
                    .add(vec4(this._invSize.xy, this._invSize.xy).mul(vec4(-0.125, -0.25, -0.125, 1.25)))
                    .toVar();
               ...
               const vOffset2 = vec4(vOffset0.xz, vOffset1.yw)
                    .add(
                         vec4(-2.0, 2.0, -2.0, 2.0)
                              .mul(vec4(this._invSize.xx, this._invSize.yy))
                              .mul(float(SMAA_MAX_SEARCH_STEPS))
                    )
                    .toVar();
          ...
          const SMAAWeightsFS = Fn(() => {
               ...
               If(e.g.greaterThan(float(0)), () => {
                    ...
                    coordsLeft.x = SMAASearchXLeft(
                         this._edgesTextureUniform,
                         this._searchTextureUniform,
                         vOffset0.xy,
                         vOffset2.x
                    );
                    ...
                    coordsRight.x = SMAASearchXRight(
                         this._edgesTextureUniform,
                         this._searchTextureUniform,
                         vOffset0.zw,
                         vOffset2.y
                    ...
                    const e2 = this._edgesTextureUniform
                         .sample(coordsRight.add(vec2(1, 0).mul(this._invSize)))
                         .r.toVar();
                    weights.r = e2;
               ...
               If(e.r.greaterThan(float(0)), () => {
                    ...
                    const e2 = this._edgesTextureUniform
                         .sample(coordsDown.add(vec2(0, 1).mul(this._invSize)))
                         .g.toVar();
