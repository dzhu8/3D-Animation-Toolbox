...
/**
 * Post processing node for denoising data like raw screen-space ambient occlusion output. Denoise can noticeably
 * improve the quality of ambient occlusion but also add quite some overhead to the post processing setup. It's best to
 * make its usage optional (e.g. via graphic settings).
 *
 * Reference:
 * {@link https://openaccess.thecvf.com/content/WACV2021/papers/Khademi_Self-Supervised_Poisson-Gaussian_Denoising_WACV_2021_paper.pdf}.
 *
 * @augments TempNode
...
class DenoiseNode extends TempNode {
     ...
     constructor(textureNode, depthNode, normalNode, camera) {
          ...
          /**
           * A node that represents the scene's normals. If no normals are passed to the constructor (because MRT is not
           * available), normals can be automatically reconstructed from depth values in the shader.
           *
           * @type {?Node<vec3>}
           */
               ...
               ([uvNode]) => {
                    ...
                    If(depth.greaterThanEqual(1.0).or(dot(viewNormal, viewNormal).equal(0.0)), () => {
                         ...
                         Loop({ start: int(0), end: int(16), type: "int", condition: "<" }, ({ i }) => {
                              ...
                              const offset = rotationMatrix
                                   .mul(sampleDir.xy.mul(float(1.0).add(sampleDir.z.mul(this.radius.sub(1)))))
 ...
 * @returns {Vector3[]} The denoise samples.
 */
 ...
 *
 * @param {number} [size=64] - The texture size. Default is `64`
 * @returns {DataTexture} The generated noise texture.
 */
...
export const denoise = (node, depthNode, normalNode, camera) =>
     nodeObject(new DenoiseNode(convertToTexture(node), nodeObject(depthNode), nodeObject(normalNode), camera));
