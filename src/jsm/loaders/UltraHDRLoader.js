 ...
 * Short format brief:
 *
 * [JPEG headers] [XMP metadata describing the MPF container and _both_ SDR and gainmap images] [Optional metadata]
 * [EXIF] [ICC Profile] [SDR image] [XMP metadata describing only the gainmap image] [Gainmap image]
 *
 * Each section is separated by a 0xFFXX byte followed by a descriptor byte (0xFFE0, 0xFFE1, 0xFFE2.) Binary image
 * storages are prefixed with a unique 0xFFD8 16-bit descriptor.
 */
...
const SRGB_TO_LINEAR = Array(1024)
     .fill(0)
     .map((_, value) => Math.pow((value / 255) * 0.9478672986 + 0.0521327014, 2.4));

/**
 * A loader for the Ultra HDR Image Format.
 *
 * Existing HDR or EXR textures can be converted to Ultra HDR with this
 * [tool]{@link https://gainmap-creator.monogrid.com/}.
 *
 * Current feature set:
 *
...
class UltraHDRLoader extends Loader {
      ...
      * @returns {RGBELoader} A reference to this loader.
      */
     setDataType(value) {
          this.type = value;
               ...
               if (sectionType === 0xe0) {
					...
					*/
                         ...
                         const gainmapImageOffset =
                              sectionData.getUint32(mpfBytesOffset + 20, mpfLittleEndian) + sectionOffset + 6;
      ...
      * @returns {DataTexture} The Ultra HDR texture.
      */
     ...
     _parseXMPMetadata(xmpDataString, xmpMetadata) {
          ...
          const xmpXml = domParser.parseFromString(
               xmpDataString.substring(xmpDataString.indexOf("<"), xmpDataString.lastIndexOf(">") + 1),
               "text/xml"
          );
     ...
     _applyGainmapToSDR(xmpMetadata, sdrBuffer, gainmapBuffer, onSuccess, onError) {
          ...
          Promise.all([getImageDataFromBuffer(sdrBuffer), getImageDataFromBuffer(gainmapBuffer)])
               .then(([sdrImage, gainmapImage]) => {
                    ...
                    for (let pixelIndex = 0; pixelIndex < sdrImageData.data.length; pixelIndex += 4) {
                         ...
                         for (let channelIndex = 0; channelIndex < 3; channelIndex++) {
                              ...
                              const logBoost =
                                   xmpMetadata.gainMapMin * (1.0 - logRecovery) + xmpMetadata.gainMapMax * logRecovery;

                              const hdrValue =
                                   (sdrValue + xmpMetadata.offsetSDR) *
                                        (logBoost * weightFactor === 0.0 ? 1.0 : Math.pow(2, logBoost * weightFactor)) -
                              ...
                              hdrBuffer[pixelIndex + channelIndex] =
                                   this.type === HalfFloatType ? DataUtils.toHalfFloat(linearHDRValue) : linearHDRValue;
                         }
                    }
