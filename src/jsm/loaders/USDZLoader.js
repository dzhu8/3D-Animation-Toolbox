...
class USDZLoader extends Loader {
      ...
      * @returns {Group} The parsed asset as a group.
      */
     parse(buffer) {
          ...
          function setTextureParams(map, data_value) {
               ...
               if (data_value["float2 inputs:scale"]) {
                    map.repeat = new Vector2().fromArray(
                         JSON.parse("[" + data_value["float2 inputs:scale"].replace(/[()]*/g, "") + "]")
                    );
               }

               if (data_value["float2 inputs:translation"]) {
                    map.offset = new Vector2().fromArray(
                         JSON.parse("[" + data_value["float2 inputs:translation"].replace(/[()]*/g, "") + "]")
                    );
               }
          }
                         ...
                         if ("float inputs:clearcoatRoughness.connect" in surface) {
                              ...
                              if ('def Shader "Transform2d_clearcoatRoughness"' in data) {
                                   setTextureParams(
                                        ...
                                        data['def Shader "Transform2d_clearcoatRoughness"']
                                   );
                              }
