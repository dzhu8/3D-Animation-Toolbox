...
class VRMLLoader extends Loader {
      ...
      * @returns {Scene} The parsed scene.
      */
     parse(data, path) {
          const nodeMap = {};
          ...
          function buildMaterialNode(node) {
               ...
               for (let i = 0, l = fields.length; i < l; i++) {
                    ...
                    switch (fieldName) {
                         ...
                         case "diffuseColor":
                              materialData.diffuseColor = new Color().setRGB(
                                   fieldValues[0],
                                   fieldValues[1],
                                   fieldValues[2],
                                   SRGBColorSpace
                         ...
                         case "emissiveColor":
                              materialData.emissiveColor = new Color().setRGB(
                                   fieldValues[0],
                                   fieldValues[1],
                                   fieldValues[2],
                                   SRGBColorSpace
                         ...
                         case "specularColor":
                              materialData.specularColor = new Color().setRGB(
                                   fieldValues[0],
                                   fieldValues[1],
                                   fieldValues[2],
                                   SRGBColorSpace
          ...
          /**
           * Vertically paints the faces interpolating between the specified colors at the specified angels. This is
           * used for the Background node, but could be applied to other nodes with multiple faces as well.
           *
           * When used with the Background node, default is directionIsDown is true if interpolating the skyColor down
           * from the Zenith. When interpolating up from the Nadir i.e. interpolating the groundColor, the
           * directionIsDown is false.
           *
           * The first angle is never specified, it is the Zenith (0 rad). Angles are specified in radians. The geometry
           * is thought a sphere, but could be anything. The color interpolation is linear along the Y axis in any
           * case.
           *
           * You must specify one more color than you have angles at the beginning of the colors array. This is the
           * color of the Zenith (the top of the shape).
           *
