 ...
 *
 * [glTF]{@link https://www.khronos.org/gltf/} (GL Transmission Format) is an [open format
 * specification]{@link https://github.com/KhronosGroup/glTF/tree/main/specification/2.0} for efficient delivery and
 ...
 *
 * `GLTFLoader` uses {@link ImageBitmapLoader} whenever possible. Be advised that image bitmaps are not automatically
 * GC-collected when they are no longer referenced, and they require special handling during the disposal process.
 *
...
class GLTFLoader extends Loader {
     ...
     /**
      * Sets the given Draco loader to this loader. Required for decoding assets compressed with the
      * `KHR_draco_mesh_compression` extension.
      *
      * @param {DRACOLoader} dracoLoader - The Draco loader to set.
      * @returns {GLTFLoader} A reference to this loader.
      */
     ...
     /**
      * Sets the given meshopt decoder. Required for decoding assets compressed with the `EXT_meshopt_compression`
      * extension.
      *
      ...
      * used by third-party code to add additional logic to the loader.
      *
      ...
      * @param {string | ArrayBuffer} data - The raw glTF data.
      * @param {string} path - The URL base path.
      * @returns {Promise<GLTFLoader~LoadObject>} A Promise that resolves with the loaded glTF when the parsing has been
      *   finished.
      */
...
class GLTFParser {
     ...
     /**
      * Counts references to shared node / Object3D resources. These resources can be reused, or "instantiated", at
      * multiple nodes in the scene hierarchy. Mesh, Camera, and Light instances are instantiated and must be marked.
      * Non-scenegraph resources (like Materials, Geometries, and Textures) can be reused directly and are not marked
      * here.
      *
      ...
      * @param {string} type
      * @param {number} index
      * @returns {Promise<Object3D | Material | THREE.Texture | AnimationClip | ArrayBuffer | Object>}
      */
     getDependency(type, index) {
      ...
      * @returns {Promise<BufferAttribute | InterleavedBufferAttribute>}
      */
      ...
      * accommodate different primitive types, defines, etc. New materials will be created if necessary, and reused from
      * a cache.
      *
      * @private
      ...
      * @returns {Promise<Group | Mesh | SkinnedMesh | Line | Points>}
      */
     loadMesh(meshIndex) {
          const parser = this;
          const json = this.json;
 ...
 * @property {AnimationClip[]} animations - An array of animation clips.
 ...
 * @property {Group[]} scenes - GlTF assets might define multiple scenes.
