 ...
 *
 * [LDraw]{@link https://ldraw.org/} (LEGO Draw) is an [open format
 * specification]{@link https://ldraw.org/article/218.html} for describing LEGO and other construction set 3D models.
 *
 * An LDraw asset (a text file usually with extension .ldr, .dat or .txt) can describe just a single construction piece,
 * or an entire model. In the case of a model the LDraw file can reference other LDraw files, which are loaded from a
 * library path set with `setPartsLibraryPath`. You usually download the LDraw official parts library, extract to a
 * folder and point setPartsLibraryPath to it.
 *
 * Library parts will be loaded by trial and error in subfolders 'parts', 'p' and 'models'. These file accesses are not
 * optimal for web environment, so a script tool has been made to pack an LDraw file with all its dependencies into a
 * single file, which loads much faster. See section 'Packing LDraw models'. The LDrawLoader example loads several
 * packed files. The official parts library is not included due to its large size.
 *
...
class LDrawLoader extends Loader {
     ...
     /**
      * This method must be called prior to `load()` unless the model to load does not reference library parts (usually
      * it will be a model with all its parts packed in a single file).
      *
      * @param {string} path - Path to library parts files to load referenced parts from. This is different from
      *   Loader.setPath, which indicates the path to load the main asset from.
      * @returns {LDrawLoader} A reference to this loader.
      */
     ...
     /**
      * Sets the conditional line material type which depends on the used renderer. Use
      * {@link LDrawConditionalLineMaterial} when using `WebGLRenderer` and {@link LDrawConditionalLineNodeMaterial} when
      * using `WebGPURenderer`.
      *
      * @param {LDrawConditionalLineMaterial.constructor | LDrawConditionalLineNodeMaterial.constructor} type - The
      *   conditional line material type.
      * @returns {LDrawLoader} A reference to this loader.
      */
      ...
      * @returns {Promise} A Promise that resolves when the preload has finished.
      */
      ...
      *
      * @param {Material[]} materials - The materials to set.
      * @returns {LDrawLoader} A reference to this loader.
      */
      ...
      *
      * @param {Material[]} materials - The materials to add.
      * @returns {LDrawLoader} A reference to this loader.
      */
     ...
     /**
      * Sets a map which maps referenced library filenames to new filenames. If a fileMap is not specified (the
      * default), library parts will be accessed by trial and error in subfolders 'parts', 'p' and 'models'.
      *
      ...
      * @returns {Material | null} The material. Returns `null` if no material has been found.
      */
      ...
      *
      * For an already loaded LDraw asset, returns the Material associated with the main color code. This method can be
      * useful to modify the main material of a model or part that exposes it.
      *
      * The main color code is the standard way to color an LDraw part. It is '16' for triangles and '24' for edges.
      * Usually a complete model will not expose the main color (that is, no part uses the code '16' at the top level,
      * because they are assigned other specific colors) An LDraw part file on the other hand will expose the code '16'
      * to be colored, and can have additional fixed colors.
      *
      * @returns {Material | null} The material. Returns `null` if no material has been found.
      */
     getMainMaterial() {
     ...
     parseColorMetaDirective(lineParser) {
          ...
          if (!edgeMaterial) {
               ...
               if (this.ConditionalLineMaterial === null) {
                    throw new Error(
                         "THREE.LDrawLoader: ConditionalLineMaterial type must be specified via .setConditionalLineMaterial()."
                    );
               }
