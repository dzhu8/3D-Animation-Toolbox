 ...
 *
 * This loader only supports the `POLYDATA` dataset format so far. Other formats (structured points, structured grid,
 * rectilinear grid, unstructured grid, appended) are not supported.
 *
 * ```js
                    ...
                    if (ele.attributes.format === "binary" && compressed) {
                         ...
                         for (let i = 0; i < blocks; i++) {
                              ...
                              for (let j = 1; j < numBytes - 1; j++) {
                                   currentBlockSize =
                                        currentBlockSize |
                                        (byteData[i * numBytes + cSizeStart + j] << (j * dataPointSize));
                              }
                         ...
                         if (section && section.DataArray) {
                              if (Array.isArray(section.DataArray)) {
                                   for (const sectionEle of section.DataArray) {
                                        sectionEle["#text"] = appendedData.slice(
                                             offsets[sectionIndex],
                                             offsets[sectionIndex + 1]
                                        );
                                        sectionEle.attributes.format = "binary";
                                        sectionIndex++;
                                   }
                              } else {
                                   section.DataArray["#text"] = appendedData.slice(
                                        offsets[sectionIndex],
                                        offsets[sectionIndex + 1]
                                   );
