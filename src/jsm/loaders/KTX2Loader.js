 ...
 * which can be quickly transcoded to a wide variety of GPU texture compression formats. While KTX 2.0 also allows other
 * hardware-specific formats, this loader does not yet parse them.
 *
 * This loader parses the KTX 2.0 container and transcodes to a supported GPU compressed texture format. The required
 * WASM transcoder and JS wrapper are available from the `examples/jsm/libs/basis` directory.
 *
...
class KTX2Loader extends Loader {
      ...
      * @returns {KTX2Loader} A reference to this loader.
      */
      ...
      * @returns {Promise} A Promise that resolves when the support has been detected.
      */
      ...
      * @returns {Promise<CompressedTexture | CompressedArrayTexture | DataTexture | Data3DTexture>}
      */
     async _createTexture(buffer, config = {}) {
          ...
          const needsTranscoder =
               container.vkFormat === VK_FORMAT_UNDEFINED || (isBasisHDR && !this.workerConfig.astcHDRSupported);
...
KTX2Loader.BasisWorker = function () {
     ...
     const OPTIONS = {
          ...
          [BasisFormat.ETC1S]: FORMAT_OPTIONS.filter((opt) => opt.basisFormat.includes(BasisFormat.ETC1S)).sort(
               (a, b) => a.priorityUASTC - b.priorityUASTC
          ),

          [BasisFormat.UASTC]: FORMAT_OPTIONS.filter((opt) => opt.basisFormat.includes(BasisFormat.UASTC)).sort(
               (a, b) => a.priorityUASTC - b.priorityUASTC
          ),

          [BasisFormat.UASTC_HDR]: FORMAT_OPTIONS.filter((opt) => opt.basisFormat.includes(BasisFormat.UASTC_HDR)).sort(
               (a, b) => a.priorityHDR - b.priorityHDR
          ),
     };
