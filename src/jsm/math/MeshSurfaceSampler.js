 ...
 *
 * Building the sampler is a one-time O(n) operation. Once built, any number of random samples may be selected in
 * O(logn) time. Memory usage is O(n).
 *
 * References:
 *
...
class MeshSurfaceSampler {
     ...
     /**
      * Specifies a vertex attribute to be used as a weight when sampling from the surface. Faces with higher weights
      * are more likely to be sampled, and those with weights of zero will not be sampled at all. For vector attributes,
      * only .x is used in sampling.
      *
      * If no weight attribute is selected, sampling is randomly distributed by area.
      *
      * @param {string} name - The attribute name.
      * @returns {MeshSurfaceSampler} A reference to this sampler.
      */
     ...
     /**
      * Processes the input geometry and prepares to return samples. Any configuration of the geometry or sampler must
      * occur before this method is called. Time complexity is O(n) for a surface with n faces.
      *
      * @returns {MeshSurfaceSampler} A reference to this sampler.
      */
     build() {
     ...
     /**
      * Selects a random point on the surface of the input geometry, returning the position and optionally the normal
      * vector, color and UV Coordinate at that point. Time complexity is O(log n) for a surface with n faces.
      *
