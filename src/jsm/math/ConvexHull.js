...
/**
 * Can be used to compute the convex hull in 3D space for a given set of points. It is primarily intended for
 * {@link ConvexGeometry}.
 *
 * This Quickhull 3D implementation is a port of [quickhull3d]{@link https://github.com/maurizzzio/quickhull3d/} by
 * Mauricio Poppe.
 *
...
class ConvexHull {
      ...
      *
      * @param {Vector3[]} points - The array of points in 3D space.
      * @returns {ConvexHull} A reference to this convex hull.
      */
     ...
     /**
      * Computes the convex hull of the given 3D object (including its descendants), accounting for the world transforms
      * of both the 3D object and its descendants.
      *
      ...
      * @returns {boolean} Whether the given point lies in the convex hull or not.
      */
     containsPoint(point) {
      ...
      * @returns {Vector3 | null} The intersection point. Returns `null` if not intersection was detected.
      */
      ...
      *
      * @param {Ray} ray - The ray to test.
      * @returns {boolean} Whether the given ray intersects with this convex hull or not.
      */
     intersectsRay(ray) {
     ...
     /**
      * Removes a vertex from the 'assigned' list of vertices and from the given face. It also makes sure that the link
      * from 'face' to the first vertex it sees in 'assigned' is linked correctly after the removal.
      *
      * @private
     ...
     /**
      * Removes all the visible vertices that a given face is able to see which are stored in the 'assigned' vertex
      * list.
      *
      * @private
      * @param {Face} face - The target face.
      * @returns {VertexNode | undefined} A reference to this convex hull.
      */
      ...
      *
      * @private
      * @param {Face[]} newFaces - The new faces.
      * @returns {ConvexHull} A reference to this convex hull.
      */
      ...
      *
      * @private
      * @returns {VertexNode | null} The next vertex to add.
      */
     _nextVertexToAdd() {
     ...
     /**
      * Computes a chain of half edges in CCW order called the 'horizon'. For an edge to be part of the horizon it must
      * join a face that can see 'eyePoint' and a face that cannot see 'eyePoint'.
      *
      * @private
      * @param {Vector3} eyePoint - The 3D-coordinates of a point.
      * @param {HalfEdge} crossEdge - The edge used to jump to the current face.
      * @param {Face} face - The current face being tested.
      * @param {HalfEdge[]} horizon - The edges that form part of the horizon in CCW order.
      * @returns {ConvexHull} A reference to this convex hull.
      */
     ...
     /**
      * Creates a face with the vertices 'eyeVertex.point', 'horizonEdge.tail' and 'horizonEdge.head' in CCW order. All
      * the half edges are created in CCW order thus the face is always pointing outside the hull.
      *
      * @private
      * @param {VertexNode} eyeVertex - The vertex that is added to the hull.
      * @param {HalfEdge} horizonEdge - A single edge of the horizon.
      * @returns {HalfEdge} The half edge whose vertex is the eyeVertex.
      */
     ...
     /**
      * Adds 'horizon.length' faces to the hull, each face will be linked with the horizon opposite face and the face on
      * the left/right.
      *
      * @private
      * @param {VertexNode} eyeVertex - The vertex that is added to the hull.
      * @param {HalfEdge[]} horizon - The horizon.
      * @returns {ConvexHull} A reference to this convex hull.
      */
      ...
      *
      * - Compute the 'horizon' which is a chain of half edges. For an edge to belong to this group it must be the edge
      *   connecting a face that can see 'eyeVertex' and a face which cannot see 'eyeVertex'.
      * - All the faces that can see 'eyeVertex' have its visible vertices removed from the assigned vertex list.
      * - A new set of faces is created with each edge of the 'horizon' and 'eyeVertex'. Each face is connected with the
      *   opposite horizon face and the face on the left/right.
...
/**
 * Represents a section bounded by a specific amount of half-edges. The current implementation assumes that a face
 * always consist of three edges.
 *
 * @private
 */
class Face {
     ...
     constructor() {
           ...
           * @private
           * @default null
           * @type {VertexNode | null}
           */
          this.outside = null; // reference to a vertex in a vertex list this face can see
          this.mark = Visible;

          /**
           * Reference to the base edge of a face. To retrieve all edges, you can use the `next` reference of the
           * current edge.
           *
           * @private
           * @default null
      ...
      *
      * @private
      * @returns {Face} A reference to this face.
      */
     compute() {
...
class HalfEdge {
      ...
      *
      * @private
      * @returns {VertexNode} The destination vertex.
      */
     head() {
          return this.vertex;
     }
      ...
      *
      * @private
      * @returns {number} The edge's length.
      */
     length() {
      ...
      *
      * @private
      * @returns {number} The square of the edge's length.
      */
     lengthSquared() {
          const head = this.head();
          const tail = this.tail();
     ...
     /**
      * Sets the twin edge of this half-edge. It also ensures that the twin reference of the given half-edge is
      * correctly set.
      *
      * @private
      * @param {HalfEdge} edge - The twin edge to set.
      * @returns {HalfEdge} A reference to this edge.
      */
     setTwin(edge) {
          this.twin = edge;
          edge.twin = this;
...
class VertexList {
      ...
      *
      * @private
      * @returns {VertexNode} The head reference.
      */
     first() {
          return this.head;
     }
      ...
      *
      * @private
      * @returns {VertexNode} The tail reference.
      */
     last() {
          return this.tail;
     }
      ...
      *
      * @private
      * @returns {VertexList} A reference to this vertex list.
      */
     clear() {
