...
class LightProbeGenerator {
     /**
      * Creates a light probe from the given (radiance) environment map. The method expects that the environment map is
      * represented as a cube texture.
      *
      * @param {CubeTexture} cubeTexture - The environment map.
      * @returns {LightProbe} The created light probe.
      */
     ...
     /**
      * Creates a light probe from the given (radiance) environment map. The method expects that the environment map is
      * represented as a cube render target.
      *
      * The cube render target must be in RGBA so `cubeRenderTarget.texture.format` must be set to {@link RGBAFormat}.
      *
      * @async
      * @param {WebGPURenderer | WebGLRenderer} renderer - The renderer.
      * @param {CubeRenderTarget | WebGLCubeRenderTarget} cubeRenderTarget - The environment map.
      * @returns {Promise<LightProbe>} A Promise that resolves with the created light probe.
      */
     static async fromCubeRenderTarget(renderer, cubeRenderTarget) {
          ...
          for (let faceIndex = 0; faceIndex < 6; faceIndex++) {
               if (renderer.isWebGLRenderer) {
                    ...
                    data = await renderer.readRenderTargetPixelsAsync(
                         cubeRenderTarget,
                         0,
                         0,
                         imageWidth,
