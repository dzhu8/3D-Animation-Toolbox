...
class ModelVisualizer {
     ...
     centerAndScaleModel(object) {
          ...
          if (maxDim > 100) {
               ...
               console.log(
                    `Model scaled down by factor ${scale.toFixed(3)} (original max dimension: ${maxDim.toFixed(2)})`
               );
          } else if (maxDim < 0.1) {
               const scale = 5 / maxDim; // Scale up very small models
               object.scale.setScalar(scale);
               console.log(
                    `Model scaled up by factor ${scale.toFixed(3)} (original max dimension: ${maxDim.toFixed(4)})`
               );
          }
