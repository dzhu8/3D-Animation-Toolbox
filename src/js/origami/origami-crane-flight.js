...
class CraneInstance {
    ...
    calculateMovementDirectionForPath(time, speed) {
        ...
        if (this.frameCount < 5) { // Use this approach for first few frames
    ...
    applyRotationConstraints(targetQuaternion) {
        ...
        const pastState = this.pastStates.length > ROTATION_CONSTRAINTS.pastStateDelayFrames 
        ...
        if (!implementConstraints) {
            ...
            roll = Math.max(-ROTATION_CONSTRAINTS.maxRollFromInitial, 
...
function setupHingeEdgeData(crane) {
    if (crane.edges.length > crane.animationSet1.hingeEdgeIndex) {
        ...
        console.log(`Set up hinge edge data for crane ${crane.id} left wing: edge ${crane.animationSet1.hingeEdgeIndex}`);
    } else {
        console.warn(`Hinge edge index ${crane.animationSet1.hingeEdgeIndex} for crane ${crane.id} left wing is out of range. Total edges: ${crane.edges.length}`);
    }
    ...
    if (crane.edges.length > crane.animationSet2.hingeEdgeIndex) {
        ...
        console.log(`Set up hinge edge data for crane ${crane.id} right wing: edge ${crane.animationSet2.hingeEdgeIndex}`);
    } else {
        console.warn(`Hinge edge index ${crane.animationSet2.hingeEdgeIndex} for crane ${crane.id} right wing is out of range. Total edges: ${crane.edges.length}`);
    }
}
...
function applyWingFlappingToCrane(crane, craneColor) {
    ...
    const set1Data = crane.animationSet1.selectedFaceIndices.length > 0 ? {
    ...
    const set2Data = crane.animationSet2.selectedFaceIndices.length > 0 ? {
