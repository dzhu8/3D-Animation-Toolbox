function initViveInterface(globals) {
     ...
     if (WEBVR.isAvailable() === false) {
          $status.html(
               "WebVR not supported by this browser<br/>see <a href='https://webvr.info/' target='_blank'>webvr.info</a> for more information."
          );
          $("#VRoptions").hide();
          return;
     }
     ...
     gui.add(variables, "stepsPerFrame")
          .min(1)
          .max(200)
          .step(1)
          .name("Num Steps Per Frame")
          .onChange(function (val) {
               globals.numSteps = val;
               $(".numStepsPerRender").val(val);
          })
          .listen();
     gui.add(variables, "scale")
          .min(0.01)
          .max(1)
          .step(0.001)
          .name("Scale")
     ...
     var positionBound = 2;
     gui.add(variables.position, "x")
          .min(-positionBound)
          .max(positionBound)
          .step(0.01)
          .name("Position X")
          .onChange(positionCallback);
     gui.add(variables.position, "z")
          .min(-positionBound)
          .max(positionBound)
          .step(0.01)
          .name("Position Y")
          .onChange(positionCallback); //z and y are flipped
     gui.add(variables.position, "y")
          .min(-positionBound)
          .max(positionBound)
          .step(0.01)
          .name("Position Z")
     ...
     function connect() {
          WEBVR.getVRDisplay(function (display) {
               ...
               if (!display) {
                    $status.html(
                         "No VR device detected.  Check that you are connected to Steam VR and your HMD has the latest firmware updates, then refresh this page."
                    );
                    $link.hide();
                    return;
               }
               $status.html(
                    "VR device detected.  Hit the button below to enter VR.  If you have problems, check that you are connected to Steam VR and your HMD has the latest firmware updates."
               );
               $("#VRoptions").show();
