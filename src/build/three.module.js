...
class PMREMGenerator {
      ...
      * Optional sigma specifies a blur radius in radians to be applied to the scene before PMREM generation. Optional
      * near and far planes ensure the scene is rendered in its entirety.
      *
      * @param {Scene} scene - The scene to be captured.
      * @param {number} [sigma=0] - The blur radius in radians. Default is `0`
      * @param {number} [near=0.1] - The near plane distance. Default is `0.1`
      * @param {number} [far=100] - The far plane distance. Default is `100`
      * @param {Object} [options={}] - The configuration options. Default is `{}`
      * @param {number} [options.size=256] - The texture size of the PMREM. Default is `256`
      * @param {Vector3} [options.renderTarget=origin] - The position of the internal cube camera that renders the
      *   scene. Default is `origin`
      * @returns {WebGLRenderTarget} The resulting PMREM.
      */
      ...
      * 1k (1024 x 512), as this matches best with the 256 x 256 cubemap output.
      *
      * @param {Texture} equirectangular - The equirectangular texture to be converted.
      * @param {WebGLRenderTarget | null} [renderTarget=null] - The render target to use. Default is `null`
      * @returns {WebGLRenderTarget} The resulting PMREM.
      */
     ...
     /**
      * Generates a PMREM from an cubemap texture, which can be either LDR or HDR. The ideal input cube size is 256 x
      * 256, as this matches best with the 256 x 256 cubemap output.
      *
     ...
     /**
      * Pre-compiles the cubemap shader. You can get faster start-up by invoking this method during your texture's
     ...
     /**
      * Disposes of the PMREMGenerator's internal memory. Note that PMREMGenerator is a static class, so you should not
      * need more than one PMREMGenerator object. If you do, calling dispose() on one of them will cause any others to
     ...
     /**
      * This is a two-pass Gaussian blur for a cubemap. Normally this is done vertically and horizontally, but this
      * breaks down on a cube. Here we apply the blur latitudinally (around the poles), and then longitudinally (towards
      * the poles) to approximate the orthogonally-separable blur. It is least accurate at the poles, but still does a
      * decent job.
      *
      * @private
...
/**
 * Uniforms of a program. Those form a tree structure with a special top-level container for the root, which you get by
 * calling 'new WebGLUniforms( gl, program )'.
 *
 * Properties of inner nodes including the top-level container:
 *
 * .seq - array of nested uniforms .map - nested uniforms by name
 *
...
class WebXRDepthSensing {
      ...
      * @returns {Mesh | null} The plane mesh.
      */
     getMesh(cameraXR) {
          if (this.texture !== null) {
               if (this.mesh === null) {
      ...
      *
      * @returns {Texture | null} The depth texture.
      */
     getDepthTexture() {
          return this.texture;
     }
...
/**
 * This class represents an abstraction of the WebXR Device API and is internally used by {@link WebGLRenderer}.
 * `WebXRManager` also provides a public interface that allows users to enable/disable XR and perform XR related tasks
 * like for instance retrieving controllers.
 *
...
class WebXRManager extends EventDispatcher {
     ...
     constructor(renderer, gl) {
          ...
          /**
           * This flag notifies the renderer to be ready for XR rendering. Set it to `true` if you are going to use XR
           * in your app.
           *
           * @default false
           * @type {boolean}
          ...
          /**
           * Returns a group representing the `target ray` space of the XR controller. Use this space for visualizing 3D
           * objects that support the user in pointing tasks like UI interaction.
           *
           * @param {number} index - The index of the controller.
           * @returns {Group} A group representing the `target ray` space.
           */
          ...
          /**
           * Returns a group representing the `grip` space of the XR controller. Use this space for visualizing 3D
           * objects that support the user in pointing tasks like UI interaction.
           *
           * Note: If you want to show something in the user's hand AND offer a pointing ray at the same time, you'll
           * want to attached the handheld object to the group returned by `getControllerGrip()` and the ray to the
           * group returned by `getController()`. The idea is to have two different groups in two different coordinate
           * spaces for the same WebXR controller.
           *
           * @param {number} index - The index of the controller.
           * @returns {Group} A group representing the `grip` space.
           */
          ...
          /**
           * Returns a group representing the `hand` space of the XR controller. Use this space for visualizing 3D
           * objects that support the user in pointing tasks like UI interaction.
           *
           * @param {number} index - The index of the controller.
           * @returns {Group} A group representing the `hand` space.
           */
           ...
           *
           * @returns {XRReferenceSpace} The XR reference space.
           */
           ...
           *
           * @returns {?(XRWebGLLayer | XRProjectionLayer)} The XR base layer.
           */
           ...
           *
           * @returns {XRWebGLBinding | null} The XR binding.
           */
           ...
           *
           * @returns {XRFrame | null} The XR frame. Returns `null` when used outside a XR session.
           */
           ...
           *
           * @returns {XRSession | null} The XR session. Returns `null` when used outside a XR session.
           */
          ...
          /**
           * After a XR session has been requested usually with one of the `*Button` modules, it is injected into the
           * renderer with this method. This method triggers the start of the actual XR rendering.
           *
           * @async
           * @param {XRSession} value - The XR session to set.
           * @returns {Promise} A Promise that resolves when the session has been set.
           */
           ...
           *
           * @returns {"opaque" | "additive" | "alpha-blend" | undefined} The environment blend mode. Returns
           *   `undefined` when used outside of a XR session.
           */
          ...
          /**
           * Assumes 2 cameras that are parallel and share an X-axis, and that the cameras' projection and world
           * matrices have already been set. And that near and far planes are identical for both cameras. Visualization
           * of this technique: https://computergraphics.stackexchange.com/a/4765
           *
          ...
          /**
           * Updates the state of the XR camera. Use this method on app level if you set cameraAutoUpdate`to`false`. The
           * method requires the non-XR camera of the scene as a parameter. The passed in camera's transformation is
           * automatically adjusted to the position of the XR camera when calling this method.
           *
          ...
          /**
           * Returns an instance of {@link ArrayCamera} which represents the XR camera of the active XR session. For each
           * view it holds a separate camera object.
           *
           * The camera's `fov` is currently not used and does not reflect the fov of the XR camera. If you need the fov
           * on app level, you have to compute in manually from the XR camera's projection matrices.
           *
           * @returns {ArrayCamera} The XR camera.
           */
           ...
           *
           * @returns {number} The amount of foveation.
           */
           ...
           *
           * @param {number} value - A number in the range `[0,1]` where `0` means no foveation (full resolution) and
           ...
           *
           * @returns {boolean} Whether depth sensing is supported or not.
           */
           ...
           *
           * @returns {Mesh} The depth sensing mesh.
           */
...
class WebGLRenderer {
     ...
     constructor(parameters = {}) {
          ...
          /**
           * A canvas where the renderer draws its output.This is automatically created by the renderer in the
           * constructor (if not provided already); you just need to add it to your page like so:
           *
           * ```js
           ...
           *
           * - `checkShaderErrors`: If it is `true`, defines whether material shader programs are checked for errors
           *   during compilation and linkage process. It may be useful to disable this check in production for
           *   performance gain. It is strongly recommended to keep these checks enabled during development. If the
           *   shader does not compile and link - it will not work and associated material will not render.
           * - `onShaderError(gl, program, glVertexShader,glFragmentShader)`: A callback function that can be used for
           *   custom error reporting. The callback receives the WebGL context, an instance of WebGLProgram as well two
           *   instances of WebGLShader representing the vertex and fragment shader. Assigning a custom function
           *   disables the default error reporting.
           *
           * @type {Object}
           */
          this.debug = {
          ...
          /**
           * If {@link WebGLRenderer#autoClear} set to `true`, whether the renderer should clear the color buffer or not.
           *
           * @default true
           * @type {boolean}
           */
          ...
          /**
           * If {@link WebGLRenderer#autoClear} set to `true`, whether the renderer should clear the depth buffer or not.
           *
           * @default true
           * @type {boolean}
           */
          ...
          /**
           * If {@link WebGLRenderer#autoClear} set to `true`, whether the renderer should clear the stencil buffer or
           * not.
           *
           * @default true
           * @type {boolean}
           ...
           *
           * Note: Sorting is used to attempt to properly render objects that have some degree of transparency. By
           * definition, sorting objects may not work in all cases. Depending on the needs of application, it may be
           * necessary to turn off sorting and use other methods to deal with transparency rendering e.g. manually
           * determining each object's rendering order.
           *
           * @default true
           * @type {boolean}
           */
          ...
          /**
           * User-defined clipping planes specified in world space. These planes apply globally. Points in space whose
           * dot product with the plane is negative are cut away.
           *
           * @type {Plane[]}
           */
          this.clippingPlanes = [];
           ...
           * dimensions. Lowering this value can result in significant performance improvements when using
           * {@link MeshPhysicalMaterial#transmission}.
           *
           * @default 1
           * @type {number}
           */
          ...
          function initGLContext() {
                ...
                *
                * - `get(extensionName:string)`: Used to check whether a WebGL extension is supported and return the
                *   extension object if available.
               ...
               /**
                * Holds a series of statistical information about the GPU memory and the rendering process. Useful for
                * debugging and monitoring.
                *
                * By default these data are reset at each render call but when having multiple render passes per frame
                * (e.g. when using post processing) it can be preferred to reset with a custom pattern. First, set
                * `autoReset` to `false`.
                *
                * ```js
           ...
           *
           * @returns {WebGL2RenderingContext} The rendering context.
           */
           ...
           *
           * @returns {WebGLContextAttributes} The rendering context attributes.
           */
           ...
           * @returns {Vector2} The renderer's size in logical pixels.
           */
           ...
           * viewport to fit that size, starting in (0, 0). Setting `updateStyle` to false prevents any style changes to
           * the output canvas.
           *
           * @param {number} width - The width in logical pixels.
           * @param {number} height - The height in logical pixels.
           * @param {boolean} [updateStyle=true] - Whether to update the `style` attribute of the canvas or not. Default
           *   is `true`
           */
           ...
           *
           * @param {Vector2} target - The method writes the result in this target object.
           * @returns {Vector2} The drawing buffer size.
           */
          ...
          /**
           * This method allows to define the drawing buffer size by specifying width, height and pixel ratio all at
           * once. The size of the drawing buffer is computed with this formula:
           *
           * ```js
           ...
           *
           * @param {Vector2} target - The method writes the result in this target object.
           * @returns {Vector2} The current viewport definition.
           */
           ...
           * @returns {Vector4} The viewport definition.
           */
           ...
           *
           * @param {number | Vector4} x - The horizontal coordinate for the lower left corner of the viewport origin in
           *   logical pixel unit. Or alternatively a four-component vector specifying all the parameters of the
           *   viewport.
           * @param {number} y - The vertical coordinate for the lower left corner of the viewport origin in logical
           *   pixel unit.
           ...
           *
           * @param {Vector4} target - The method writes the result in this target object.
           * @returns {Vector4} The scissor region.
           */
           ...
           *
           * @param {number | Vector4} x - The horizontal coordinate for the lower left corner of the scissor region
           *   origin in logical pixel unit. Or alternatively a four-component vector specifying all the parameters of
           *   the scissor region.
           * @param {number} y - The vertical coordinate for the lower left corner of the scissor region origin in
           *   logical pixel unit.
           ...
           *
           * @returns {boolean} Whether the scissor test is enabled or not.
           */
          ...
          /**
           * Enable or disable the scissor test. When this is enabled, only the pixels within the defined scissor area
          ...
          /**
           * Sets a custom opaque sort function for the render lists. Pass `null` to use the default `painterSortStable`
           * function.
           *
           * @param {Function | null} method - The opaque sort function.
           */
          ...
          /**
           * Sets a custom transparent sort function for the render lists. Pass `null` to use the default
           ...
           * @param {number} [alpha=1] - The clear alpha. Default is `1`
           */
          ...
          /**
           * Tells the renderer to clear its color, depth or stencil drawing buffer(s). This method initializes the
           * buffers to the current clear color values.
           *
           * @param {boolean} [color=true] - Whether the color buffer should be cleared or not. Default is `true`
           * @param {boolean} [depth=true] - Whether the depth buffer should be cleared or not. Default is `true`
           * @param {boolean} [stencil=true] - Whether the stencil buffer should be cleared or not. Default is `true`
           */
           ...
           * rendering. If you want to add a 3D object to an existing scene, use the third optional parameter for
           * applying the target scene.
           *
           * Note that the (target) scene's lighting and environment must be configured before calling this method.
           *
           * @param {Object3D} scene - The scene or another type of 3D object to precompile.
           * @param {Camera} camera - The camera.
           * @param {Scene | null} [targetScene=null] - The target scene. Default is `null`
           * @returns {Set<Material>} The precompiled materials.
           */
           ...
           *
           * This method makes use of the `KHR_parallel_shader_compile` WebGL extension. Hence, it is recommended to use
           * this version of `compile()` whenever possible.
           *
           * @async
           * @param {Object3D} scene - The scene or another type of 3D object to precompile.
           * @param {Camera} camera - The camera.
           * @param {Scene | null} [targetScene=null] - The target scene. Default is `null`
           * @returns {Promise} A Promise that resolves when the given scene can be rendered without unnecessary
           *   stalling due to shader compilation.
           */
           ...
           * {@link WebGLRenderer#setRenderTarget} or to the canvas as usual.
           *
           * By default render buffers are cleared before rendering but you can prevent this by setting the property
           * `autoClear` to `false`. If you want to prevent only certain buffers being cleared you can `autoClearColor`,
           * `autoClearDepth` or `autoClearStencil` to `false`. To force a clear, use {@link WebGLRenderer#clear}.
           *
           ...
           *
           * @returns {number} The active cube face.
           */
           ...
           *
           * @returns {number} The active mipmap level.
           */
           ...
           *
           * @returns {WebGLRenderTarget | null} The active render target. Returns `null` if no render target is
           *   currently set.
           */
           ...
           *
           * @param {WebGLRenderTarget | null} renderTarget - The render target to set. When `null` is given, the canvas
           *   is set as the active render target instead.
           * @param {number} [activeCubeFace=0] - The active cube face when using a cube render target. Indicates the z
           *   layer to render in to when using 3D or array render targets. Default is `0`
           * @param {number} [activeMipmapLevel=0] - The active mipmap level. Default is `0`
           */
           ...
           * @param {number} [textureIndex=0] - The texture index of an MRT render target. Default is `0`
           */
           ...
           * @param {number} [activeCubeFaceIndex] - The active cube face index.
           * @param {number} [textureIndex=0] - The texture index of an MRT render target. Default is `0`
           * @returns {Promise<TypedArray>} A Promise that resolves when the read has been finished. The resolve
           *   provides the read data as a typed array.
           */
           ...
           * @param {Vector2 | null} [position=null] - The start position of the copy operation. Default is `null`
           * @param {number} [level=0] - The mip level. The default represents the base mip. Default is `0`
           */
           ...
           *
           * When using render target textures as `srcTexture` and `dstTexture`, you must make sure both render targets
           * are initialized {@link WebGLRenderer#initRenderTarget}.
           *
           * @param {Texture} srcTexture - The source texture.
           * @param {Texture} dstTexture - The destination texture.
           * @param {?(Box2 | Box3)} [srcRegion=null] - A bounding box which describes the source region. Can be two or
           *   three-dimensional. Default is `null`
           * @param {?(Vector2 | Vector3)} [dstPosition=null] - A vector that represents the origin of the destination
           *   region. Can be two or three-dimensional. Default is `null`
           * @param {number} [srcLevel=0] - The source mipmap level to copy. Default is `0`
           * @param {number | null} [dstLevel=null] - The destination mipmap level. Default is `null`
           */
          ...
          /**
           * Initializes the given WebGLRenderTarget memory. Useful for initializing a render target so data can be
           * copied into it using {@link WebGLRenderer#copyTextureToTexture} before it has been rendered to.
           *
          ...
          /**
           * Can be used to reset the internal WebGL state. This method is mostly relevant for applications which share
